...
export async function up({ db, payload, req }: MigrateUpArgs): Promise<void> {
  ...
  await db.run(sql`CREATE TABLE \`vendure_product\` (
  ...
  `)
  await db.run(sql`CREATE INDEX \`vendure_product_updated_at_idx\` ON \`vendure_product\` (\`updated_at\`);`)
  await db.run(sql`CREATE INDEX \`vendure_product_created_at_idx\` ON \`vendure_product\` (\`created_at\`);`)
  await db.run(sql`CREATE TABLE \`vendure_product_rels\` (
  	\`id\` integer PRIMARY KEY NOT NULL,
  	\`order\` integer,
  ...
  await db.run(sql`CREATE TABLE \`vendure_product_variant\` (
  	\`id\` numeric PRIMARY KEY NOT NULL,
  	\`name\` text NOT NULL,
  	\`slug\` text NOT NULL,
  	\`product_id\` numeric,
  	\`updated_at\` text DEFAULT (strftime('%Y-%m-%dT%H:%M:%fZ', 'now')) NOT NULL,
  	\`created_at\` text DEFAULT (strftime('%Y-%m-%dT%H:%M:%fZ', 'now')) NOT NULL,
  	FOREIGN KEY (\`product_id\`) REFERENCES \`vendure_product\`(\`id\`) ON UPDATE no action ON DELETE set null
  );
  `)
  await db.run(sql`CREATE INDEX \`vendure_product_variant_product_idx\` ON \`vendure_product_variant\` (\`product_id\`);`)
  await db.run(sql`CREATE INDEX \`vendure_product_variant_updated_at_idx\` ON \`vendure_product_variant\` (\`updated_at\`);`)
  await db.run(sql`CREATE INDEX \`vendure_product_variant_created_at_idx\` ON \`vendure_product_variant\` (\`created_at\`);`)
...
export async function down({ db, payload, req }: MigrateDownArgs): Promise<void> {
  ...
  await db.run(sql`DROP TABLE \`vendure_product\`;`)
  await db.run(sql`DROP TABLE \`vendure_product_rels\`;`)
  await db.run(sql`DROP TABLE \`vendure_product_variant\`;`)

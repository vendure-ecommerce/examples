...
export class CmsSyncService implements OnApplicationBootstrap {
  ...
  // Implemented in the cms-specific service
  ensureContentTypesExists() {
    this.cmsService.ensureStoryContentTypesExists();
  }
  ...
  async syncProductToCms(jobData: SyncJobData): Promise<SyncResponse> {
    try {
      ...
      // Implemented in the cms-specific service
      await this.cmsService.syncProduct({
        product,
        defaultLanguageCode,
        operationType,
        productSlug,
  ...
  async syncVariantToCms(jobData: SyncJobData): Promise<SyncResponse> {
    try {
      ...
      // Implemented in the cms-specific service
      await this.cmsService.syncProductVariant({
        variant,
        defaultLanguageCode,
        operationType,
        variantSlug,
  ...
  async syncCollectionToCms(jobData: SyncJobData): Promise<SyncResponse> {
    try {
      ...
      // Implemented in the cms-specific service
      await this.cmsService.syncCollection({
        collection,
        defaultLanguageCode,
        operationType,
        collectionSlug,

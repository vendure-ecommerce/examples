...
import { authenticateCustomer, authenticateWithGoogle, updateCustomer } from '@/lib/vendure';
...
export async function signInWithGoogle(token: string): Promise<SignInState> {
  if (!token) {
    return {
      type: 'error',
